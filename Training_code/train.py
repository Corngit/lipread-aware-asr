...
def main():
    ...
    trainData = LRS2Main(f"fold{fold}_train", args["DATA_DIRECTORY"], args["MAIN_REQ_INPUT_LENGTH"], args["CHAR_TO_INDEX"], args["STEP_SIZE"],
                         audioParams, videoParams, noiseParams)
    trainLoader = DataLoader(trainData, batch_size=args["BATCH_SIZE"], collate_fn=collate_fn, shuffle=False, **kwargs)
    noiseParams = {"noiseFile":args["DATA_DIRECTORY"] + "/noise.wav", "noiseProb":0, "noiseSNR":args["NOISE_SNR_DB"]}
    valData = LRS2Main(f"fold{fold}_val", args["DATA_DIRECTORY"], args["MAIN_REQ_INPUT_LENGTH"], args["CHAR_TO_INDEX"], args["STEP_SIZE"],
                       audioParams, videoParams, noiseParams)
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=args["LR_SCHEDULER_FACTOR"],
                                                     ...
                                                     threshold_mode="abs", min_lr=args["FINAL_LR"])
    ...
    for step in range(args["NUM_STEPS"]):
        ...
        if ((step%args["SAVE_FREQUENCY"] == 0) or (step == args["NUM_STEPS"]-1)) and (step != 0):
            ...
            savePath = args["CODE_DIRECTORY"] + "/checkpoints/models/fold{}-step_{:04d}-cer_{:.3f}-wer_{:.3f}.pt".format(args["FOLD"],step,validationCER, validationWER)
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints/plots/fold{}_train-step_{:04d}-loss.png".format(args["FOLD"],step))
            plt.close()
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints/plots/fold{}_train-step_{:04d}-wer.png".format(args["FOLD"],step))
            plt.close()
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints/plots/fold{}_train-step_{:04d}-cer.png".format(args["FOLD"],step))
            plt.close()
